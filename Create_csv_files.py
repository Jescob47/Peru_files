...
workk_directory = 'D:/OneDrive - Universidad EAFIT/U/proyectos julian/Programacion/Prueba pagina peru/'
...
class CreateCSVFiles(unittest.TestCase):
    def organize_download_zip(self):
        ...
        shutil.move(download_directory + file_name, workk_directory + file_name)
        with ZipFile(file_name, 'r') as zip:
            zip.extractall(csv_directory)
            print('File is unzipped in folder %s' %csv_directory)
        self.assertTrue(os.path.isdir(workk_directory + csv_directory))
        remove(file_name)
